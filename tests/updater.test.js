...
test('updater skips when disabled in config unless forced by env', () => {
    ...
    assert.equal(updater.shouldSkip(['node', 'src/index.ts'], {}).skip, true)
    assert.equal(updater.shouldSkip(['node', 'src/index.ts'], { MSRB_AUTO_UPDATE: '1' }).skip, false)
})

test('resolveUpdateSettings reads repo from config and package.json', () => {
    const root = tempRoot()
    ...
    fs.writeFileSync(
        ...
        JSON.stringify({
            version: '1.0.0',
            repository: { url: 'https://github.com/acme/my-bot.git' }
        })
    )
    fs.writeFileSync(
    ...
    const { resolveUpdateSettings } = require('../scripts/updater/resolveUpdateSettings')
    const settings = resolveUpdateSettings({ root, env: {} })

    assert.equal(settings.enabled, true)
    assert.equal(settings.repo, 'custom/user-repo')
    assert.equal(settings.branch, 'release')
})
