...
const { resolveUpdateSettings } = require('./updater/resolveUpdateSettings')
...
async function main() {
    ...
    const settings = resolveUpdateSettings({ root: ROOT })
    const updater = new UpdateManager({ root: ROOT })

    console.log(`[UPDATE-DOCTOR] package.json version: ${packageJson.version}`)
    console.log(`[UPDATE-DOCTOR] auto-update enabled: ${settings.enabled}`)
