...
const { resolveUpdateSettings } = require('./resolveUpdateSettings')
...
function githubAuthHeaders(env = process.env) {
    const token = env.MSRB_GITHUB_TOKEN || env.GITHUB_TOKEN
    if (!token) {
        return {}
    }
...
class UpdateManager {
    constructor(options = {}) {
        ...
        const settings = resolveUpdateSettings({ root: this.root, env: options.env ?? process.env })
        this.updatesEnabled = options.updatesEnabled ?? settings.enabled
        this.repo = options.repo ?? settings.repo
        this.branch = options.branch ?? settings.branch
    ...
    shouldSkip(argv = process.argv, env = process.env) {
        if (env.MSRB_AUTO_UPDATE === '0' || env.MSRB_AUTO_UPDATE === 'false') {
            return { skip: true, reason: 'MSRB_AUTO_UPDATE=0' }
        }
        if (!this.updatesEnabled && env.MSRB_AUTO_UPDATE !== '1' && env.MSRB_AUTO_UPDATE !== 'true') {
            return { skip: true, reason: 'updates disabled in config' }
        }
        if (env.NODE_CLUSTER_WORKER) return { skip: true, reason: 'cluster worker' }
        ...
        if (env.npm_lifecycle_event === 'ts-start') return { skip: true, reason: 'ts-start' }
        return { skip: false }
    }

    getGithubHeaders(env = process.env) {
        return githubAuthHeaders(env)
    }
    ...
    async run(options = {}) {
        ...
        try {
            const remote = await this.fetchRemoteRelease(env)
    ...
    async fetchRemoteRelease(env = process.env) {
        const authHeaders = this.getGithubHeaders(env)
        const branchUrl = this.githubApiUrl(`/repos/${this.repo}/branches/${encodeURIComponent(this.branch)}`)
        const branch = await requestJson(branchUrl, 20_000, authHeaders)
        ...
        const packageSource = await requestText(packageUrl, 20_000, {
    ...
    async applyRelease(remote, env = process.env) {
