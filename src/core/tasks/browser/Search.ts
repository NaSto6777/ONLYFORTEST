...
import { detectBingSearchBlock, type BingSearchBlockInfo } from '../../../automation/RewardsPageAnalyzer'
...
import {
    ...
    clearAccountSearchIssues,
    clearAccountTempBan
} from '../../../helpers/AccountTempBanLedger'
...
/** Consecutive searches with no points before marking the account search-limited for today. */
const NO_POINTS_SEARCH_LIMIT_THRESHOLD = 5
...
export class Search extends TaskBase {
    ...
    public async doSearch(data: DashboardData, page: Page, isMobile: boolean): Promise<number> {
        ...
        const tempBanned = accountEmail ? isAccountTempBannedToday(accountEmail) : false
        const recheckingTempBan = tempBanned && this.bot.localRunIgnoreTempBan
        if (tempBanned) {
            if (!this.bot.localRunIgnoreTempBan) {
                this.bot.logger.warn(
                    isMobile,
                    'SEARCH-BING',
                    `Skipping Bing searches — account temp-banned for today | account=${accountEmail}`
                )
                return 0
            }

            clearAccountTempBan(accountEmail)
            this.bot.logger.info(
                isMobile,
                'SEARCH-BING',
                `Re-checking search limit — retrying Bing searches | account=${accountEmail}`
            )
        }
        ...
        try {
            ...
            let stagnantLoop = 0
            const stagnantLoopMax = NO_POINTS_SEARCH_LIMIT_THRESHOLD
            ...
            for (let i = 0; i < queries.length; i++) {
                ...
                const searchResult = await this.bingSearch(page, query, isMobile)
                if (searchResult.blocked) {
                    searchBlocked = true
                    break
                }

                searchCounters = searchResult.counters
                ...
                if (stagnantLoop >= stagnantLoopMax && missingPointsTotal > 0) {
                    searchBlocked = this.markSearchLimitedFromNoPoints(page, isMobile, stagnantLoopMax)
                    break
                }
            ...
            if (missingPointsTotal > 0 && !searchBlocked) {
                this.bot.logger.info(
                    isMobile,
                    'SEARCH-BING',
            ...
            if (accountEmail && !searchBlocked) {
                if (totalGainedPoints > 0) {
                    clearAccountSearchIssues(accountEmail)
                } else if (recheckingTempBan) {
                    this.bot.logger.info(
                        isMobile,
                        'SEARCH-BING',
                        `Search limit re-check passed — account is not temp-banned | account=${accountEmail}`
                    )
                }
            } else if (accountEmail && searchBlocked && recheckingTempBan) {
                this.bot.logger.warn(
                    isMobile,
                    'SEARCH-BING',
                    `Search limit re-check failed — account is still temp-banned | account=${accountEmail}`
                )
            }
    ...
    private markSearchLimitedFromNoPoints(page: Page, isMobile: boolean, streak: number): boolean {
        this.markAccountTempBanned(
            {
                blocked: true,
                reason: `No search points after ${streak} consecutive searches`,
                url: page.url(),
                title: ''
            },
            isMobile
        ...
        this.bot.logger.warn(
            isMobile,
            'SEARCH-BING',
            `Search-limited for today after ${streak} searches with 0 points earned`
        )
        return true
    }

    private markAccountTempBanned(blockInfo: BingSearchBlockInfo, isMobile: boolean): void {
        const accountEmail = getCurrentContext().account.email
        if (!accountEmail) {
            return
        }

        if (!isAccountTempBannedToday(accountEmail)) {
            recordAccountSearchIssue({
                email: accountEmail,
                kind: 'temp_ban',
        ...
        const suffix = this.bot.localRunIgnoreTempBan ? ' (re-check confirmed)' : ''
        this.bot.logger.warn(
            isMobile,
            'SEARCH-BING',
            `Account temp-banned for today — stopping all Bing searches | account=${accountEmail} | reason=${blockInfo.reason}${suffix}`
        )
    }

    private markAccountNeedsSignIn(blockInfo: BingSearchBlockInfo, isMobile: boolean): void {
        const accountEmail = getCurrentContext().account.email
        if (!accountEmail) {
            return
        }
        ...
        this.bot.logger.warn(
            isMobile,
            'SEARCH-BING',
            `Bing sign-in needed — open the account in Rewards Desk to sign in locally | account=${accountEmail} | url=${blockInfo.url}`
        )
    }

    private handleSearchBlock(blockInfo: BingSearchBlockInfo, isMobile: boolean): 'stop' | 'retry' {
        ...
        this.bot.logger.warn(
            isMobile,
            'SEARCH-BING',
            `Bing search restricted for this account | reason=${blockInfo.reason} | url=${blockInfo.url}`
        )

        if (blockInfo.reason === 'Bing sign-in required') {
            this.markAccountNeedsSignIn(blockInfo, isMobile)
            return 'stop'
        }
    ...
    private shouldStopSearchForPageBlock(blockInfo: BingSearchBlockInfo): boolean {
    ...
    private async ensureBingSearchContext(page: Page, isMobile: boolean): Promise<void> {
        ...
        if (!host.endsWith('bing.com') || host === 'rewards.bing.com') {
            this.bot.logger.debug(
                isMobile,
                'SEARCH-BING',
                `Restoring Bing search page before inspection | from=${page.url()}`
            )
            await page.goto(this.bingHome, { waitUntil: 'domcontentloaded', timeout: 20_000 }).catch(() => {})
            await page.waitForLoadState('networkidle', { timeout: 10_000 }).catch(() => {})
            await this.bot.browser.utils.tryDismissAllMessages(page)
        }
    }

    private async bingSearch(searchPage: Page, query: string, isMobile: boolean): Promise<BingSearchResult> {
        const maxAttempts = 5
        ...
        for (let i = 0; i < maxAttempts; i++) {
            try {
                if (i > 0) {
                    await searchPage.goto(this.bingHome, { waitUntil: 'domcontentloaded', timeout: 20_000 }).catch(() => {})
                    await searchPage.waitForLoadState('networkidle', { timeout: 10_000 }).catch(() => {})
                    await this.bot.browser.utils.tryDismissAllMessages(searchPage)
                }

                await this.ensureBingSearchContext(searchPage, isMobile)
                const blockInfo = await this.inspectBingSearchPage(searchPage)
                if (this.shouldStopSearchForPageBlock(blockInfo)) {
                    this.handleSearchBlock(blockInfo, isMobile)
                    return {
                ...
                const searchSelector =
                    (await searchPage.locator(BING_SEARCH.searchBar).count().catch(() => 0)) > 0
                        ? BING_SEARCH.searchBar
                ...
                await this.bot.browser.utils.ghostClick(searchPage, searchSelector, { clickCount: 3 })
                ...
                return { counters, blocked: false }
            } catch (error) {
                await this.ensureBingSearchContext(searchPage, isMobile).catch(() => {})
                const blockInfo = await this.inspectBingSearchPage(searchPage).catch(() => null)
                if (blockInfo && this.shouldStopSearchForPageBlock(blockInfo)) {
                    this.bot.logger.warn(
                        isMobile,
                        'SEARCH-BING',
                        `Stopping search retries — ${blockInfo.reason} | query="${query}"`
                    )
                    this.handleSearchBlock(blockInfo, isMobile)
                    return {
                        counters: await this.bot.browser.func.getSearchPoints(),
    ...
    private async resolveSearchBox(page: Page) {
        const primary = page.locator(BING_SEARCH.searchBar)
        if ((await primary.count().catch(() => 0)) > 0) {
            return primary
        }

        return page.locator('input[name="q"], textarea[name="q"]')
    }

    private async inspectBingSearchPage(page: Page) {
        const [html, meta] = await Promise.all([
            page.content().catch(() => ''),
            page.evaluate(() => ({ url: location.href, title: document.title })).catch(() => ({
                url: page.url(),
                title: ''
            }))
        ])

        return detectBingSearchBlock(html, meta.url, meta.title)
    }
