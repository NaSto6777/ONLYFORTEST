...
import { isAccountTempBannedToday, clearAccountTempBan } from '../helpers/AccountTempBanLedger'
...
export class SearchOrchestrator {
    ...
    async doSearches(
        ...
        const tempBanned = isAccountTempBannedToday(accountEmail)
        if (tempBanned) {
            if (!this.bot.localRunIgnoreTempBan) {
                this.bot.logger.warn(
                    'main',
                    'SEARCH-MANAGER',
                    `Skipping all searches — account temp-banned for today | account=${accountEmail}`
                )
            ...
            this.bot.logger.info(
                'main',
                'SEARCH-MANAGER',
                `Re-checking search limit — cleared temp-ban flag and retrying searches | account=${accountEmail}`
            )
        }
