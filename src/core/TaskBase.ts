...
export class TaskBase {
    ...
    protected async waitActivityCooldown(): Promise<void> {
        const delay =
            this.bot.promotionRunActive && this.bot.config.promotionDelay
                ? this.bot.config.promotionDelay
                : { min: 5000, max: 15000 }

        await this.bot.utils.wait(this.bot.utils.randomDelay(delay.min, delay.max))
    }
    ...
    public async doDailySet(data: DashboardData, page: Page) {
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'DAILY-SET',
            `Started solving ${activitiesUncompleted.length} Daily Set item(s)`
        )
    ...
    public async doMorePromotions(data: DashboardData, page: Page) {
        ...
        this.bot.promotionRunActive = true
        try {
            await this.solveActivities(activitiesUncompleted, page)
        } finally {
            this.bot.promotionRunActive = false
        }
    ...
    public async doAppPromotions(data: AppDashboardData) {
        ...
        this.bot.logger.info(
            this.bot.isMobile,
            'APP-PROMOTIONS',
            `Started solving ${appRewards.length} sapphire app promotion(s)`
        )
    ...
    private async solveActivities(activities: BasePromotion[], page: Page, punchCard?: PunchCard) {
        for (const activity of activities) {
            try {
                ...
                // Cooldown
                await this.waitActivityCooldown()
            } catch (error) {
                this.bot.logger.error(
                    this.bot.isMobile,
                    'ACTIVITY',
