...
import { formatConsoleLog } from '../helpers/TerminalLogFormatter'
...
export class LogService {
    ...
    private baseLog(
        ...
        const verboseLogs = config.debugLogs || process.argv.includes('-dev') || process.env.MSRB_VERBOSE_LOGS === '1'
        const formattedConsole = formatConsoleLog({
            level,
            platform: isMobile,
            userName,
            title,
        ...
        if (consoleAllowed && !formattedConsole.suppress) {
            if (logColor && !formattedConsole.line.includes('\x1b[')) {
                consoleOut(level, formattedConsole.line, getColorFn(logColor))
            } else {
                consoleOut(level, formattedConsole.line, null)
            }
        }
