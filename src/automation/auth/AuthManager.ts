...
export class AuthManager {
    ...
    async login(page: Page, account: Account) {
        try {
            ...
            while (iteration < maxIterations) {
                ...
                if (pageUrl.hostname === 'rewards.bing.com' && pageUrl.pathname.startsWith('/welcome')) {
                    await this.triggerInteractiveSignIn(page, account)
                    await this.bot.utils.wait(2000)
                    continue
                }
                ...
                if (state === 'LOGGED_IN') {
                    ...
                    if (!this.isRewardsAuthenticatedUrl(authenticatedUrl)) {
                        this.bot.logger.warn(
                            this.bot.isMobile,
                            'LOGIN',
                            `LOGGED_IN rejected — ${authenticatedUrl.pathname} is not an authenticated Rewards page`
                        )
                        if (
                            authenticatedUrl.hostname === 'rewards.bing.com' &&
                            authenticatedUrl.pathname.startsWith('/welcome')
                        ) {
                            await this.triggerInteractiveSignIn(page, account)
                            await this.bot.utils.wait(2000)
                        }
                    ...
                    if (!emailEntered && !passwordEntered) {
                        this.bot.logger.debug(
                            this.bot.isMobile,
                            'LOGIN',
                            'Authenticated Rewards page reached with restored session cookies'
                        )
                    }
    ...
    private async detectCurrentState(page: Page, account?: Account): Promise<LoginState> {
        ...
        if (this.isRewardsAuthenticatedUrl(url)) {
            this.bot.logger.debug(this.bot.isMobile, 'DETECT-STATE', 'On authenticated Rewards/account page')
            return 'LOGGED_IN'
        }
        ...
        if (url.hostname === 'rewards.bing.com' && url.pathname.startsWith('/welcome')) {
            this.bot.logger.debug(
                this.bot.isMobile,
                'DETECT-STATE',
                'Rewards welcome/onboarding page — not authenticated yet'
            )
        }
    ...
    private isRewardsAuthenticatedUrl(url: URL): boolean {
        ...
        const unauthenticatedPrefixes = ['/welcome', '/signin', '/signin-oidc', '/signup', '/join']
        if (unauthenticatedPrefixes.some(prefix => url.pathname.toLowerCase().startsWith(prefix))) {
            return false
        }

        const authenticatedPrefixes = ['/dashboard', '/earn', '/redeem', '/status', '/pointsbreakdown', '/account']
        return authenticatedPrefixes.some(
            prefix => url.pathname === prefix || url.pathname.startsWith(`${prefix}/`)
        )
    }

    private isInteractiveLoginState(state: LoginState): boolean {
        return !['LOGGED_IN', 'UNKNOWN', 'CHROMEWEBDATA_ERROR', 'ACCOUNT_LOCKED'].includes(state)
    }

    private async triggerInteractiveSignIn(page: Page, account: Account): Promise<void> {
        this.bot.logger.warn(
            this.bot.isMobile,
            'LOGIN',
            'On Rewards welcome/unauthenticated page — opening Microsoft sign-in'
        )

        const loginHint = encodeURIComponent(account.email)
        const wreply = encodeURIComponent('https://rewards.bing.com/dashboard')
        await page
            .goto(
                `https://login.live.com/login.srf?wa=wsignin1.0&wreply=${wreply}&id=293577&login_hint=${loginHint}`,
    ...
    private async finalizeLogin(page: Page, account: Account) {
        ...
        const rewardsUrl = new URL(page.url())
        if (this.isRewardsAuthenticatedUrl(rewardsUrl)) {
            this.bot.logger.info(this.bot.isMobile, 'LOGIN', 'Logged into Microsoft Rewards successfully')
        } else if (rewardsUrl.hostname === 'rewards.bing.com' && rewardsUrl.pathname.startsWith('/welcome')) {
            throw new Error(
                'Login incomplete: Rewards welcome page detected. Enroll in Microsoft Rewards manually in a browser first, or use a supported region.'
            )
        } else {
        ...
        await this.ensureRewardsWebSession(page, account)
        ...
        try {
            ...
            for (let i = 0; i < loopMax; i++) {
                ...
                if (state === 'PASSKEY_ERROR') {
                    ...
                    await this.bot.browser.utils.ghostClick(page, this.selectors.secondaryButton)
                } else if (account && this.isInteractiveLoginState(state)) {
                    this.bot.logger.info(
                        this.bot.isMobile,
                        'LOGIN-BING',
                        `Handling login state during Bing verification: ${state}`
                    )
                    const shouldContinue = await this.handleState(state, page, account)
                    if (!shouldContinue) {
                        throw new Error(`Bing login failed at state: ${state}`)
                    }
                    await this.bot.utils.wait(1000)
                    continue
                }
    ...
    async ensureRewardsWebSession(page: Page, account: Account): Promise<void> {
        this.bot.logger.info(this.bot.isMobile, 'REWARDS-SESSION', 'Verifying Rewards web session')

        await page.goto(this.bot.config.baseURL, { waitUntil: 'networkidle', timeout: 30_000 }).catch(() => {})

        const rewardsUrl = new URL(page.url())
        if (rewardsUrl.hostname === 'rewards.bing.com' && rewardsUrl.pathname.startsWith('/welcome')) {
            throw new Error(
                'Login incomplete: Rewards welcome page detected. Enroll in Microsoft Rewards manually in a browser first, or use a supported region.'
            )
        }

        if (await this.validateRewardsWebSession(page)) {
            await this.getRewardsSession(page)
            return
        }

        // Dashboard not ready yet — wait for client-side hydration before deciding the session is bad.
        await this.bot.utils.wait(3000)
        if (await this.validateRewardsWebSession(page)) {
            await this.getRewardsSession(page)
            return
        ...
        if (this.isRewardsAuthenticatedUrl(currentUrl) && !(await this.isOnSignInPage(page))) {
            this.bot.logger.warn(
                this.bot.isMobile,
                'REWARDS-SESSION',
                'Rewards dashboard is open but getuserinfo API is unavailable — continuing with browser session'
            )
            await this.getRewardsSession(page)
            return
        }
        ...
        this.bot.logger.warn(
            this.bot.isMobile,
            'REWARDS-SESSION',
            'Rewards session not ready — opening sign-in without clearing saved session'
        )
        await this.triggerInteractiveSignIn(page, account)
        await page.goto(this.bot.config.baseURL, { waitUntil: 'networkidle', timeout: 30_000 }).catch(() => {})
        ...
        if (!(await this.validateRewardsWebSession(page))) {
            throw new StaleRewardsSessionError(
                account.email,
                `Rewards web session could not be established for ${account.email}`
            )
        }
    ...
    private async isOnSignInPage(page: Page): Promise<boolean> {
        const url = page.url()
        if (/login\.(live|microsoftonline)\.com|signin|oauth/i.test(url)) {
            return true
        }

        return page
            .evaluate(() => /sign in to microsoft rewards/i.test(document.title))
            .catch(() => false)
    }

    private async isRewardsDashboardReady(page: Page): Promise<boolean> {
        const url = new URL(page.url())
        if (url.hostname !== 'rewards.bing.com' || url.pathname.startsWith('/welcome')) {
            return false
        }
        ...
        return page
            .evaluate(() => {
                if (/signin|login|oauth|\/welcome/i.test(location.href)) {
                    return false
                }

                return Boolean(
                    document.querySelector('section#snapshot, section#dailyset, #balanceToolTipDiv, [data-testid="userBalance"]')
                )
            })
            .catch(() => false)
    }

    private async validateRewardsWebSession(page: Page, retries = 2): Promise<boolean> {
        if (await this.isRewardsDashboardReady(page)) {
            this.bot.logger.debug(this.bot.isMobile, 'REWARDS-SESSION', 'Rewards dashboard loaded in browser')
            return true
        }

        return this.validateRewardsDashboardApi(page, retries)
    }

    private async validateRewardsDashboardApi(page: Page, retries = 3): Promise<boolean> {
        for (let attempt = 0; attempt < retries; attempt++) {
            const valid = await page
                .evaluate(async (apiUrl: string) => {
                    try {
                        ...
                        const payload = JSON.parse(text) as { dashboard?: { userStatus?: unknown } }
                        return Boolean(payload?.dashboard?.userStatus)
                    } catch {
                        return false
                    }
                }, URLS.dashboardApi)
            ...
            if (valid) {
                this.bot.logger.debug(this.bot.isMobile, 'REWARDS-SESSION', 'Rewards API session is valid')
                return true
            }
