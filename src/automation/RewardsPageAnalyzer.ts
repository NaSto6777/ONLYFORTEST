...
export interface BingSearchBlockInfo {
    blocked: boolean
    reason: string
    url: string
    title: string
...
export function detectBingSearchBlock(html: string, url: string, title = ''): BingSearchBlockInfo {
    ...
    const hasSearchBox = /sb_form_q|name="q"|id="sb_form"/i.test(html)
    const onLoginHost = /^(login|account)\.live\.com$|login\.microsoftonline\.com$/i.test(host)

    // Rewards dashboard is not a Bing search surface.
    if (host === 'rewards.bing.com') {
        return { ...base, blocked: false, reason: '' }
    }

    // Ignore unrelated hosts unless we landed on Microsoft login.
    if (!host.endsWith('bing.com') && !onLoginHost) {
        return { ...base, blocked: false, reason: '' }
    }

    if (/captcha|turnstile|hcaptcha|recaptcha|verify you are human|are you a robot/i.test(`${html} ${title}`)) {
        return { ...base, blocked: true, reason: 'CAPTCHA / human verification page' }
    }

    if (/unusual activity|automated queries|suspicious activity|temporarily blocked/i.test(`${html} ${title}`)) {
        return { ...base, blocked: true, reason: 'Bing unusual-activity / search restriction (possible temp ban)' }
    }

    if (/access denied|rate limit|too many requests|service unavailable/i.test(`${html} ${title}`)) {
        return { ...base, blocked: true, reason: 'Bing rate limit or access restriction' }
    }

    if (onLoginHost || (/www\.bing\.com$/i.test(host) && /sign in to (microsoft )?bing|sign in to your account/i.test(`${html} ${title}`) && !hasSearchBox)) {
        return { ...base, blocked: true, reason: 'Bing sign-in required' }
    }

    if (/www\.bing\.com$/i.test(host) && !hasSearchBox) {
        return { ...base, blocked: true, reason: 'Bing search box missing — page may be blocked or failed to load' }
    }
...
export function extractDashboardServerActionIds(text: string, functionName: string): string[] {
    const ids: string[] = []
    const escaped = functionName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    const forward = new RegExp(
        `createServerReference\\)?\\s*\\(\\s*"([a-f0-9]{40,64})"[^)]*${escaped}`,
        'gi'
    )
    const reverse = new RegExp(
        `${escaped}[\\s\\S]{0,300}?createServerReference\\)?\\s*\\(\\s*"([a-f0-9]{40,64})"`,
        'gi'
    )
...
export function extractReportActivityActionIds(text: string): string[] {
    return extractDashboardServerActionIds(text, 'reportActivity')
}
