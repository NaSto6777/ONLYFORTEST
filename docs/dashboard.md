...
Navigation: [Documentation index](./README.md) -> [Official Core plugin](./core-plugin.md) -> [Core technical reference](./core-plugin-reference.md) -> [Local control panel](./local-control-panel.md) -> [Troubleshooting](./troubleshooting.md)
...
The open-source bot ships a separate **localhost control panel** (`controlPanel.enabled` in `config.json`). See [Local control panel](./local-control-panel.md) for setup. That panel is not the Core remote dashboard.
...
| --- | --- | --- |
| Local HTTP control panel (`127.0.0.1`) | Yes | No |
| Remote web dashboard | No | Yes |
| License + Discord login | No | Yes |
| Masked account status | Partial (no email masking) | Yes |
| Recent filtered logs | Yes (local panel) | Yes |
| Safe remote actions | Local panel only | Yes |
| Safe config overrides | Local panel (allowlisted keys) | Yes |
| Account editor | No passwords via panel | Yes, encrypted to the local bot |
| Background auto-start | Control panel keep-alive | Yes |
