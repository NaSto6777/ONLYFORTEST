...
- `MSRB_AUTO_UPDATE=1`: force update checks even when `updates.enabled` is false in config.
- `MSRB_UPDATE_CHECK_ONLY=1`: check and log only; do not apply updates.
- `MSRB_UPDATE_REPO=your-user/Microsoft-Rewards-Bot`: override the GitHub repo.
- `MSRB_UPDATE_BRANCH=main`: override the update branch.
- `MSRB_GITHUB_TOKEN` or `GITHUB_TOKEN`: optional token for higher GitHub API rate limits.

## Your Own GitHub Repo

Add this to `src/config.json` (or set `repository.url` in `package.json`):

```json
"updates": {
    "enabled": true,
    "repo": "your-github-username/Microsoft-Rewards-Bot",
    "branch": "main"
}
```

Push each release to that branch with a bumped `package.json` `version`. `npm start` and `npm run farm` check GitHub before build, download when newer, preserve `config.json`, `accounts.json`, and `sessions/`, then rebuild.
